document.addEventListener('alpine:init', () => {
    Alpine.data('qfHTMLForms', () => ({
        ...
        formTextTemplate: '<div class="col-sm-[[size]]"><label for="[[name]]" class="form-label">[[label]]</label><input type="[[type]]" class="form-control" id="[[name2]]" [[required]] value="[[value]]" placeholder="[[placeholder]]"></div>',
        formDateTemplate: '<div class="col-sm-[[size]]"><label for="[[name]]" class="form-label">[[label]]</label><input type="[[type]]" class="form-control" id="[[name2]]" [[required]] value="[[value]]" placeholder="[[placeholder]]"></div>',
        formTextAreaTemplate: '<div class="col-sm-[[size]]"><label for="[[name]]" class="form-label">[[label]]</label><textarea class="form-control" id="[[name2]]" placeholder="[[placeholder]]" [[required]]>[[value]]</textarea></div>',
        formEmptyTemplate: '<div class="col-sm-[[size]]"></div>',
        formSelectTemplate: '<div class="col-sm-[[size]]"><label for="[[name]]" class="form-label">[[label]]</label><select class="form-select" id="[[name2]]" [[required]]><option disabled value="">Choose One...</option>[[options]]</select></div>',
        ...
        buildRelationField(inField, inValue = []) {
            ...
            // get the field name for the relation link in the table
            var fieldID = parseInt(inField.relation.field);
            var fieldName = this.getCurrentProject().tables[parseInt(inField.relation.table)].formJSON[fieldID].name;
            
            var currentDataRowSelectedValue = -1;
            if (inValue.length === 1) {
                currentDataRowSelectedValue = inValue[0].qfid;
            }
            var optionsList = '';
            for (var j = 0; j < dataList.length; j++) {
                optionsList += '<option value="' + dataList[j].qfid + '"';
                if (dataList[j].qfid === currentDataRowSelectedValue) {
                    optionsList += ' selected>';
                } else {
                    optionsList += '>';
                }
                optionsList += dataList[j][fieldName] + '</option>';
            }
        ...
        buildFieldHTML(formJSONItem, inValue = '') {
            ...
            if (formJSONItem['required'] === 'yes') 
                fieldRequired = true;
            if (fieldType == 'text' || fieldType == 'email' || fieldType == 'password' || fieldType == 'file' || fieldType == 'date' || fieldType == 'array' || fieldType == 'url' || fieldType == 'number') {
                newField = this.buildTextField(formJSONItem, inValue, fieldRequired);
            }
            ...
            if (fieldType == 'relation') {
                newField = this.buildRelationField(formJSONItem, inValue);
            }
            if (fieldRequired)
                newField = newField.replace('[[required]]', 'required');
            else
                newField = newField.replace('[[required]]', '');
            return newField;
        },
